...
For example, this CSV associates the `UK_current` locale food `197UPFRL` with the nutrient table record `8445` in the `UK_NDB_3` nutrient table:

```csv
"Locale","Food code","FCT (NDB name)","FCT record ID (NDB Food Code)"
"UK_current","197UPFRL","UK_NDB_3","8445"
```
