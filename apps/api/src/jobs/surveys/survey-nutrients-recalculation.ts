...
export default class SurveyNutrientsRecalculation extends BaseJob<'SurveyNutrientsRecalculation'> {
  ...
  private async recalculate(batchSize = BATCH_SIZE) {
    ...
    for (const offset of offsets) {
      ...
      // Fetch current food mappings if needed (skip codes already cached from previous batches)
      if (flags.shouldUpdateNutrientCodes || flags.shouldSyncFields) {
        for (const [locale, localeFoods] of localeGroups) {
          const foodCodes = [...new Set(localeFoods.map(f => f.code))]
            .filter(code => !currentMappings.has(`${locale}:${code}`));
      ...
      // Prepare nutrient table lookups — only for foods not already covered by currentMappings
      ...
      for (const food of foods) {
        ...
        // Skip nutrient record lookup if currentMapping already provides the data
        if (flags.shouldUpdateNutrientCodes) {
          const currentMapping = currentMappings.get(`${food.locale}:${food.code}`);
        ...
        if (!nutrientTableRecordIds[food.nutrientTableId]) {
          nutrientTableRecordIds[food.nutrientTableId] = new Set();
        }
        nutrientTableRecordIds[food.nutrientTableId].add(food.nutrientTableCode);
      }
