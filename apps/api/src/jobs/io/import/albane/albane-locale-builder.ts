import type {
  ...
  AlbaneFacetsRow,
  AlbaneFoodCategoryRow,
  AlbaneFoodListRow,
  AlbanePortionSizeImage,
...
const DUMMY_NUTRIENT_TABLE: PkgV2NutrientTable = {
  id: 'FR_TEMP',
  ...
  records: [
    {
      ...
      recordId: 'FRPH1',
      nutrients: [['1', 100]] as [string, number][],
      fields: [],
    },
  ],
};
...
// Images to ignore to avoid creating duplicates
...
export interface AlbaneConversionResult {
...
export class AlbaneLocaleBuilder {
  ...
  private portionSizeMethods: Record<string, PkgV2PortionSizeMethod[]> | undefined;
  ...
  private async readFoodCategories(): Promise<void> {
    ...
    const categoryRecords = await this.readXLSX<AlbaneFoodCategoryRow>('CATEGORIES_I24_FOOD.xlsx');
  ...
  private buildFoods(): PkgV2Food[] {
    ...
    for (const row of this.sourceFoodRecords!) {
      ...
      // Build alternativeNames only if we have synonyms, must be non-empty tuples
      const alternativeNames: PkgV2AltNames = {};
      if (foodSynonyms && foodSynonyms.length > 0) {
        const truncated = foodSynonyms.map(s => s.substring(0, 128)).filter(s => s.length > 0);
        if (truncated.length > 0) {
          // Cast to [string, ...string[]] since we've verified length > 0
          alternativeNames.fr = truncated as [string, ...string[]];
        }
      }
      ...
      // Add orderBy to each portion size method and associated food
      const portionSizeWithOrder = portionSizeMethods.map((psm, idx) => ({
        ...psm,
        orderBy: String(idx),
      }));

      const associatedFoodsForFood = this.associatedFoodPrompts![row.A_CODE] ?? [];
      const associatedFoodsWithOrder = associatedFoodsForFood.map((af, idx) => ({
        ...af,
        orderBy: String(idx),
      }));
      ...
      foods.push({
        ...
        associatedFoods: associatedFoodsWithOrder,
        parentCategories: categories,
        portionSize: portionSizeWithOrder,
        brandNames: [],
      });
    }
    ...
    if (this.categoryNames !== undefined) {
      for (const [code, name] of Object.entries(this.categoryNames)) {
        if (categories.some(cat => cat.code === code)) {
          this.logger.warn(`Category code ${code} is a built-in Albane category, skipping translation from CATEGORIES_I24_LIST file`);
          continue;
        }
  ...
  private async readAssociatedFoodPrompts(): Promise<void> {
    ...
    const afpRows = await this.readXLSX<AlbaneAfpRow>('ASSOCIATED_FOOD_PROMPTS.xlsx', {
      header: ['code', 'label', 'categoryCode1', 'promptText1', 'categoryCode2', 'promptText2', 'categoryCode3', 'promptText3', 'genericName'],
      range: 1,
    });
    ...
    for (let i = 0; i < afpRows.length; i++) {
      ...
      const prompts: PkgV2AssociatedFood[] = [];

      // Just sense checking
      if (!row.code) {
        this.logger.warn(`Food code column empty in row ${i + 1} of the associated food prompts file`);
        continue;
      }

      if (!row.genericName) {
        this.logger.warn(`Generic name column empty in row ${i + 1} (Albane food code ${row.code}) of the associated food prompts file`);
        continue;
      }

      if (row.categoryCode1) {
        if (!row.promptText1) {
          this.logger.warn(`Category code defined but prompt text 1 column is empty in row ${i + 1} (Albane food code ${row.code}) of the associated food prompts file`);
          continue;
        }
      ...
      if (row.categoryCode2) {
        if (!row.promptText2) {
          this.logger.warn(`Category code defined but prompt text 2 column is empty in row ${i + 1} (Albane food code ${row.code}) of the associated food prompts file`);
          continue;
        }
      ...
      if (row.categoryCode3) {
        if (!row.promptText3) {
          this.logger.warn(`Category code defined but prompt text 3 column is empty in row ${i + 1} (Albane food code ${row.code}) of the associated food prompts file`);
          continue;
        }
  ...
  private async readFacetFlags(): Promise<void> {
    const facetsRows = await this.readXLSX<AlbaneFacetsRow>('FACETS.xlsx');
  ...
  private async readPortionSizeImages(): Promise<void> {
    ...
    const standardUnitRows = await this.readXLSX<Record<number, string>>('List.Photos_HHM_Shapes.xlsx', {
      header: 1,
      range: 1,
  ...
  private async readStandardUnits(): Promise<Dictionary<PkgV2StandardPortionPsm>> {
    const getStandardUnits = (rows: AlbaneStandardUnitRow[]): PkgV2StandardUnit[] => {
    ...
    const standardUnitRows = (await this.readXLSX<AlbaneStandardUnitRow>('US.xlsx')).filter(row => !!row.A_CODE);
  ...
  private async readPortionSizeMethods(): Promise<void> {
    ...
    const quantificationRows = (await this.readXLSX<AlbaneQuantificationRow>('FDQUANT.xlsx')).filter(row => !!row.A_CODE);

    const standardUnits = await this.readStandardUnits();

    for (const row of quantificationRows) {
      const foodCode = row.A_CODE;
      const foodPortionSizeMethods = new Array<PkgV2PortionSizeMethod>();
      ...
      if (row.LISTE_photos) {
        ...
        if (ids.length !== conversionFactors.length) {
          this.logger.error(`LISTE_photos has ${ids.length} values but VALUERS_conversion_photos has ${conversionFactors.length} values for food ${row.A_CODE}, they must have the same length`);
          continue;
        }
  ...
  private buildAsServed(): PkgV2AsServedSet[] {
    ...
    return Object.entries(imagesById).map(([imageId, images]) => {
      const label = images[0].intake24Instructions ? { fr: images[0].intake24Instructions } : null;
      return {
  ...
  private buildPortionSizeImageLabels(): AlbaneConversionResult['portionSizeImageLabels'] {
  ...
  private buildHouseholdMeasuresMap(): void {
  ...
  public async buildPackage(): Promise<AlbaneConversionResult> {
