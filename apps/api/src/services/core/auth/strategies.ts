...
export function buildJwtStrategy(frontEnd: FrontEnd): Strategy {
  return new Strategy(opts[frontEnd], async (req: Request, payload: TokenPayload, done) => {
    ...
    if (!aud.includes('personal')) {
      ...
      console.log(`non pat token, setting aal to`, req.aal);
      done(null, payload);
      return;
    }
    ...
    try {
      ...
      console.log(`pat token, setting aal to`, req.aal, mfa.mode === 'optional', payload.aal === 'aal2', mfa.compat && token.createdAt < mfa.compat);
      done(null, payload);
    }
    catch (err) {
      done(err, false);
