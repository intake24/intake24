...
import { MediableContract } from '@intake24/common/contracts';
...
import type { FeedbackScheme, ModelStatic, SurveyScheme } from '@intake24/db';

export function mediable(securable: ModelStatic<FeedbackScheme | SurveyScheme>, contract: MediableContract) {
  ...
  return initServer().router(contract, {
    browse: {
      ...
      handler: async ({ query, req }) => {
        ...
        const media = await mediaService.browseMedia({ modelType, modelId }, query);
    ...
    store: {
      ...
      handler: async ({ body, file, req }) => {
        ...
        const media = await mediaService.createMedia({ modelType, modelId }, body, data);
    ...
    update: {
      ...
      handler: async ({ body, params, req }) => {
        ...
        await aclService.findAndCheckRecordAccess(securable, 'media', { attributes: ['id'], where: { id: modelId } });
        const media = await mediaService.updateMedia({ id: mediaId, modelType, modelId }, body);
