              ...
              <v-row>
                ...
                <v-col cols="12" md="6">
                  <select-icon
                    ...
                    clearable
                    :error-messages="errors.get(`${dialog.index}.icon`)"
                    :name="`${dialog.index}.icon`"
                    @update:model-value="errors.clear(`${dialog.index}.icon`)"
                  />
                </v-col>
                <v-col cols="12" md="6">
                  <v-switch
                    v-model="dialog.item.exclusive"
                    :error-messages="errors.get(`${dialog.index}.exclusive`)"
                    :label="$t('locales.food-builders.exclusive')"
                    :name="`${dialog.index}.exclusive`"
                  />
                </v-col>
                <v-col cols="12">
                  <v-select
                ...
                <v-col cols="12">
                  <language-selector
                    v-model="dialog.item.label"
                    border
                    :label="$t('locales.food-builders.label')"
                  >
                    <template v-for="lang in Object.keys(dialog.item.label)" :key="lang" #[`lang.${lang}`]>
                      <v-text-field
                        v-model="dialog.item.label[lang]"
                        :error-messages="errors.get(`${dialog.index}.label.${lang}`)"
                        :name="`${dialog.index}.label.${lang}`"
                        @update:model-value="errors.clear(`${dialog.index}.label.${lang}`)"
                      />
                    </template>
                  </language-selector>
                </v-col>
...
import { foodBuilderTypes, foodStepDefaults, foodStepTypes } from '@intake24/common/types/http/admin';
...
function addStep() {
  dialog.value.item.steps.push({ ...copy(foodStepDefaults.ingredient), id: randomString(6) });
}
...
function updateStepType(stepIndex: number) {
  const step = foodStepDefaults[dialog.value.item.steps[stepIndex].type];
  if (!step)
    return;
