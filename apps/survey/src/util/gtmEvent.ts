...
export function sendPrePostSurveyLinkEvent(
  event: 'postSurveyLinkShown' | 'postSurveyLinkClicked' | 'preSurveyLinkShown' | 'preSurveyLinkClicked',
  link_kind: 'dietary_feedback' | 'follow_up',
): void {
  sendGtmEvent({
    event,
    link_kind,
    noninteraction: event.endsWith('Shown'),
  });
}
