...
describe('sendGtmEvent', async () => {
  ...
  it.each(['dietary_feedback', 'follow_up'] as const)(
    ...
    (linkKind) => {
      gtmEvent.sendPrePostSurveyLinkEvent('postSurveyLinkShown', linkKind);
  ...
  it('marks pre-survey link impressions as non-interactions', () => {
    gtmEvent.sendPrePostSurveyLinkEvent('preSurveyLinkShown', 'dietary_feedback');

    expect(trackEvent).toHaveBeenCalledWith(expect.objectContaining({
      event: 'preSurveyLinkShown',
      link_kind: 'dietary_feedback',
      noninteraction: true,
    }));
  });
  ...
  it.each(['dietary_feedback', 'follow_up'] as const)(
    ...
    (linkKind) => {
      gtmEvent.sendPrePostSurveyLinkEvent('postSurveyLinkClicked', linkKind);
