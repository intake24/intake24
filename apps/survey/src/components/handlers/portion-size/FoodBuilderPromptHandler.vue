...
function resolveIngredients(parent?: FoodState): (EncodedFood | MissingFood)[] {
  return state.value.steps.reduce<(EncodedFood | MissingFood)[]>((acc, step, idx) => {
    if (step.type !== 'ingredient')
      return acc;

    const stepData = food.value.template.steps[idx];
    if (stepData.type !== 'ingredient')
      return acc;

    const foods: (EncodedFood | MissingFood)[] = step.foods.map((item) => {
