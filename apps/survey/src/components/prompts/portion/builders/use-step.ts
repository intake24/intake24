...
import type { UserCategoryData, UserFoodData } from '@intake24/common/types/http/foods';
...
export function createStepProps<S extends FoodBuilderStepType>() {
  return {
    ...
    entities: {
      type: Object as PropType<{ categories: UserCategoryData[]; foods: UserFoodData[] }>,
      default: () => ({ categories: [], foods: [] }),
    },
    localeId: {
      type: String,
      required: true,
