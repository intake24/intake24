...
export default {
  up: (queryInterface, Sequelize) =>
    queryInterface.sequelize.transaction(async (transaction) => {
      // Update user_survey_sessions
      const userSessionCount = await queryInterface.sequelize.query(
        `SELECT count(id) FROM user_survey_sessions where created_at > now() - INTERVAL '5 DAY'`,
        { type: Sequelize.QueryTypes.SELECT, transaction },
      );

      const userSessionTotal = userSessionCount[0].count;
      const userSessionBatchSize = 50;
      const userSessionOffsets = [];
      let userSessionStart = 0;

      while (userSessionStart < userSessionTotal) {
        userSessionOffsets.push(userSessionStart);
        userSessionStart += userSessionBatchSize;
      }

      for (const userSessionOffset of userSessionOffsets) {
        const difference = userSessionBatchSize + userSessionOffset - userSessionTotal;
        console.log(`Processing offset: ${userSessionOffset}, difference: ${difference}`);
        ...
        `, {
          ...
          replacements: {
            limit: difference > 0 ? userSessionBatchSize - difference : userSessionBatchSize,
            offset: userSessionOffset,
          },
          transaction,
        });
        ...
        for (const session of sessions) {
          const newSession = cloneDeep(session.session_data);
          const state = mapSession(session.session_data);

          for (const meal of Object.values(state.meals)) {
            for (const food of Object.values(meal.foods)) {
              if (food.type !== 'encoded-food' || !food.portionSize)
                continue;

              const { conversionFactor } = food.data.portionSizeMethods[food.portionSizeMethodIndex];
              const { method, servingWeight, leftoversWeight } = food.portionSize;
              const path = food.path.replace('state.', '');
              const psmPath = `${path}.portionSize`;

              set(newSession, `${psmPath}.conversionFactor`, conversionFactor);

              if (conversionFactor !== 1 && ['as-served', 'cereal', 'drink-scale', 'milk-on-cereal', 'pizza'].includes(method)) {
                const newServingWeight = typeof servingWeight === 'number' ? (servingWeight * conversionFactor) : servingWeight;
                const newLeftoversWeight = typeof leftoversWeight === 'number' ? (leftoversWeight * conversionFactor) : leftoversWeight;

                // patch the session object with new weights
                set(newSession, `${psmPath}.servingWeight`, newServingWeight);
                set(newSession, `${psmPath}.leftoversWeight`, newLeftoversWeight);
              }
            }
          }

          sessionsToUpdate.push({ id: session.id, params: newSession });
        }

        if (sessionsToUpdate.length) {
          const sessionValues = sessionsToUpdate.map((s) => {
            const id = queryInterface.sequelize.escape(s.id);
            const params = `${queryInterface.sequelize.escape(JSON.stringify(s.params))}::jsonb`;
            return `(${id}, ${params})`;
          }).join(',');

          await queryInterface.sequelize.query(
            `UPDATE user_survey_sessions SET session_data = data.params::jsonb
              FROM (VALUES ${sessionValues}) AS data(id, params)
              WHERE data.id::uuid = user_survey_sessions.id;`,
            { transaction },
          );
        }
      }

      // Update jobs and survey_submission_foods
      ...
      for (const offset of offsets) {
        ...
        for (const submission of submissions) {
          ...
          const foodsToUpdate = dbFoods.reduce((acc, dbFood) => {
            ...
            */
            ...
            if (!sessionFood) {
              const sessionFood2 = Object.values(state.meals[mealIndex].foods)
                .find((f) => {
                  if (f.type === 'encoded-food' && f.data.code === dbFood.code
                    && dbFood.portion_size_method_id === f.portionSize?.method
                    && !isEqual(f.portionSize, dbFood.portion_size)
                    && Number(f.portionSize.servingWeight) === Number(dbFood.portion_size.servingWeight)
                    && Number(f.portionSize.leftoversWeight) === Number(dbFood.portion_size.leftoversWeight)
                    && !foodPathAlreadyIncluded.includes(f.path)) {
                    return true;
                  }
              ...
              console.log('--- need to update food');
              console.log(dbFood.id);
              console.log(JSON.stringify(dbFood.portion_size));
              console.log(JSON.stringify(sessionFood2.portionSize));
            ...
            */
            ...
            const sessionDataPsm = sessionFood.data.portionSizeMethods[sessionFood.portionSizeMethodIndex];
            if (!sessionDataPsm)
              throw new Error(`Portion size method not found for food: ${dbFood.id}, session: ${state.uxSessionId}`);

            const { conversionFactor } = sessionDataPsm;
            ...
            const updateConversionFactor = conversionFactor !== 1 && ['as-served', 'cereal', 'drink-scale', 'milk-on-cereal', 'pizza'].includes(dbFood.portion_size_method_id);

            let newPsm;

            if (updateConversionFactor) {
              const servingWeight = foodPsm.servingWeight * conversionFactor;
              const leftoversWeight = foodPsm.leftoversWeight * conversionFactor;

              newPsm = { ...cloneDeep(foodPsm), conversionFactor, servingWeight, leftoversWeight };
              ...
              newPsm = { ...cloneDeep(foodPsm), conversionFactor };
            }

            // patch the session object with added conversion factor
            set(newSession, `${path}.conversionFactor`, conversionFactor);
          ...
          sessionsToUpdate.push({ id: submission.session_id, params: newSession });
        ...
        if (sessionsToUpdate.length) {
          ...
          await queryInterface.sequelize.query(
            `UPDATE jobs SET params = data.params::jsonb
              FROM (VALUES ${sessionValues}) AS data(id, params)
              WHERE data.id::uuid = jobs.session_id;`,
            { transaction },
          );
        }
      }
