    ...
    queryInterface.sequelize.transaction(async (transaction) => {
      ...
      for (const offset of offsets) {
        ...
        for (const submission of submissions) {
          ...
          await queryInterface.sequelize.query(
            `INSERT INTO ssf_cf (id, session_path, conversion_factor, orig_nutrients, orig_portion_size, orig_session) VALUES ${insertValues.join(',')};`,
            { transaction },
          );

          console.log(`Inserted conversion factor records for session: ${state.uxSessionId}, submission id: ${submission.id}, records inserted: ${insertValues.length}`);
        }
      }
