...
describe('formatZodError', () => {
  ...
  describe('grouping depth', () => {
    it('groups errors by path prefix with groupingDepth', () => {
      ...
      if (!result.success) {
        ...
        const formatted = formatZodError(result.error, { groupingDepth: 2 });
        expect(formatted).toHaveLength(1);
  ...
  describe('discriminated union handling', () => {
    it('formats discriminated union errors with clear message', () => {
      ...
      const result = animalSchema.safeParse({ type: 'bird', feathers: true });
      expect(result.success).toBe(false);
    ...
    it('handles portion size method-like discriminated unions', () => {
      ...
      const result = portionSizeMethodSchema.safeParse({
  ...
  describe('formatZodErrorAsString', () => {
    it('returns a formatted string with all errors', () => {
      ...
      const result = schema.safeParse({ name: 123, email: 'not-an-email' });
      expect(result.success).toBe(false);

      if (!result.success) {
        const formatted = formatZodErrorAsString(result.error);
    ...
    it('formats grouped errors with path prefix', () => {
      ...
      if (!result.success) {
        const formatted = formatZodErrorAsString(result.error, { groupingDepth: 1 });
